...
public class KindsOfVariable {//클래스 영역의 시작
    ...
    * 클래스 영역에서 작성하는 변수를 필드라고 한다.
    * 필드==멤버변수(클래스가 가지는 멤버라는 의미)==전역변수(클래스 전역에서 사용할 수 있는 변수라는 의미)
    * */

    /*non-static field를 인스턴스변수라고 한다. (인스턴스 생성 시점에 사용가능한 변수라는 의미)*/
    private int globalNum;

    /*static field 정적필드(클래스변수)라고 한다. (정적(클래스) 영역에 생성되는 변수라는 의미이다.)*/
    ...
    public void testMethod1(int num){
        ...
        * */
        ...
        System.out.println(num);        //매개변수는 호출 시 값이 넘어와서 변경되기ㅣ 떄문에 초기화가 필요 없다.

        /*지역변수는 선언 외에 다시 사용(호출)하기 위해서는 반듸 초기화가 되어야 한다.*/
