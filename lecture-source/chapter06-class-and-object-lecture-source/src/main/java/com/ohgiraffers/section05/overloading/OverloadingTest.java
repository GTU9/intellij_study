...
public class OverloadingTest {
    ...
    * 이때, 동일한 기느의 메소들르 매개변수에 따라 다르게 이름을 정의하면 복잡하고 관리하기가 매우 어려울 것이다.
    ...
    * 한 클래스 내에 동일한 이름의 메소들을 여러 개 작성할 수 있도록 한다.
    * 메소드의 시그니쳐(Signature)가 다르면 다른 메소드로 인식하기 때문이다.
    * 즉, 시그니쳐 중 메소드 이름은 동일해야 하기 때문에 파라미터 선언부가 다르게 작성되어야 오버로딩이 성립된다.
    * */

    /*메소드의 시그니쳐란?
    * public void method(int num){} 이라는 메소드의 메소드명과 파라미터 선언부 분분을 시그니쳐라고 부른다.
    * method(int num)이 부분이 시그니쳐이다.
    * 메소드의 시그니처가 달라야 하기 떄문에 접근제한자나 반환형은 오버로딩 성립요건에 해당하지 않는다.
    * */
...
//    public int test(){return 0;}  //반환형은 메소드 시그니처에 해당하지 않는다.(에러남)
    ...
    public void test(int num){}     //파리미터 선언부는 메소드 시그니처에 해당한다.
