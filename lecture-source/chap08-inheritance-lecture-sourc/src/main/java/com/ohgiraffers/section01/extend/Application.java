...
public class Application {
    public static void main(String[] args) {
        ...
        * 클래스 또한 부모클래스와 자식클래스로 역할을 나누어서 부모가 가지는 멤버를 자시이 물려받아
        ...
        * 메소드 재정의를 하면 메소드 호출 시 재정의한 메소드가 우선적으로 동작하게 된다.
        *
        * 이런한 상속이라는 기술을 사용하게 되면 얻게되는 이점은 크게 두 가지로 볼 수 있다.
        * 1. 새로운 클래스를 작성할 시 기존에 작성한 클래스를 재사용할 수 있다.
        *   1-1. 재상용시 생산성을 크게 향상시킬 수 있다.(새롭게 작성하는거보다 빠르다.)
        ...
        *    상속 구조가 복잡해 질 수록 그 영향에 대한 예측이 힘들며 이런 단점이 유지보수성이 증가한다는 장점과는 반대로
        *    유지보수에 악영향을 미친다.
        * 2. 또한 부모클래스의  변경 또한 쉽지 않다. 자식클래스에서 중요하게 사용하는 기능인 경우
        *    부모클래스를 변경할 시 자식 클래스에 모두 영향을 줄 수 있다.
        *    역시 유지 보수에 악영향을 미친다.
        * 3. 부목클래스에서는 의미 있었던 기능이 자식클래스에서는 무의미할 수 있다. (불필요한 기능이 추가됨)
        *
        * 장점과 단점을 고려했을 때,
        * 상속은 재상용이라는 장접만 바라보게 되면 오용의 가능성이 있기 때문에 유지보숙에 좋지 않은 코드를 작성할 확룰이 높다.
        ...
        * 부모클래스를 추상화 하는 경우에는 역할의 관으로 바라봐야 한다.
