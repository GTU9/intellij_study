...
//        model.addAttribute("flashMessage1","redirect model message");
        ...
        * 단, 세션에 임시로 값을 담았다가 소멸하는 방식이므로 세션에 동일한 키 값이 존재하지 않아야 한다.*/
        rttr.addFlashAttribute("flashMessage1", "redirect model message (string return)");
        return "redirect:/";
    }

    @GetMapping("modelandview")
    public ModelAndView modleAndViewReturning(ModelAndView mv){
...
//        mv.addObject("flashMessage2","redirect model message (string return)");
