...
public class FirstController {
    ...
    public String registMenu(Model model, HttpServletRequest request){
        ...
        String message=name+"을(를) 신메뉴 목록의 "+categoryCode+"번 카테고리에 "+price+"원으로 둥록하였습니다.";
    ...
    /*2.@RequestParam을 이용하여 요청 파라미터 전달 받기*/
    @PostMapping("modify")
    public String modifyMenu(Model model,
                             @RequestParam(required = false) String modifyName,     //앞단의 이름과 같아야 함 @RequestParam 생략가능하지만 명시하는게 좋음
                             @RequestParam(defaultValue = "0") int modifyPrice ){   //default 값 설정
    ...
    public String modifyAllByMap(Model model, @RequestParam Map<String,String> parameters){
    ...
    *       클래스 레벨에 @SessionAttribute 어노테이션을 이용하여 세션을 담을 key 값을 설정하면
    *       Model 영역에 해당 key로 attribute가 추가되는 경우 세션에 자동 등록함*/
    ...
    public String LogoutTest2(SessionStatus sessionStatus){
        ...
        * SessionAttributes로 등록된 값은 session의 상태를 관리하는 SessionStatus의
        setComplete() 메소드를 호출해야 사용이 만려되며,
        etComplete() 메소드는 세션에 저장된 모든 정보를 제거함(개별 제거 불가)
        * */
