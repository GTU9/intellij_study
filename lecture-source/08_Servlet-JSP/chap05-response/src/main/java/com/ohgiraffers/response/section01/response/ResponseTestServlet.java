...
public class ResponseTestServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        * 2. 비즈니스 로직 처리= DB접속 및 CRUD 로직 처리 -> Service를 호출하는 쪽으로 해결(MVC)한다.
...
//        PrintWriter out = response.getWriter();
        ...
        /*브라우저로 내보낼 데이터 타입을 응답 헤더에 설정하는데,
        * content-type 헤더 값은  null이 기본 값이므로 content-type 설정을 해줘야 한다.*/
        System.out.println("default response type : " + response.getContentType());

        /*text/plain으로 설정하면 html 태그를 태가가 아닌 문자열로 인식한다.*/
